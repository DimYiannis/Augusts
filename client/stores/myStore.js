    ...
    async login() {
      try {
        const response = await axios.get(
          "http://localhost:5000/api/v1/users/showUser",  { withCredentials: true }
        );
...
export const useModalsStore = defineStore("modal", {
  ...
  actions: {
    ...
    async fetchFavItems() {
      ...
      try {
        const response = await axios.get("http://localhost:5000/api/v1/likes", { withCredentials: true });

        console.log("Favorite items fetched successfully:", response.data);

        this.favItems = Array.isArray(response.data.items) ? response.data.items : [];
      } catch (error) {
    ...
    async addToFav(item) {
      ...
      try {
        const response = await axios.post("http://localhost:5000/api/v1/likes", item, { withCredentials: true });
    ...
    async removeFromFav(item) {
      ...
      try {
        await axios.delete(`http://localhost:5000/api/v1/likes/${item.id}`, { withCredentials: true });
  ...
  actions: {
    async fetchCartItems() {
      ...
      try {
        ...
        this.cartItems = Array.isArray(response.data.items) ? response.data.items : [];
      } catch (error) {
    ...
    async addToCart(item) {
      this.isAddingToCart = true; // Set the flag to indicate loading state
      try {
        // Send POST request to add the item to the server cart
        const response = await axios.post("http://localhost:5000/api/v1/cart", item,{ withCredentials: true });
    ...
    async removeCartItem(itemId) {
      ...
      try {
        ...
        await axios.delete(`http://localhost:5000/api/v1/cart/${itemId}`, { withCredentials: true });
