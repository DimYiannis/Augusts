    ...
    <div class="fav bg-white rounded-lg shadow-lg overflow-hidden">
      ...
      <div class="chosenitems p-4">
        <div class="h-full relative">
          ...
          <div v-if="isFetchingFav || isRemovingFromFav" class="absolute inset-0 flex items-center justify-center">
          ...
          <div v-else-if="!favItems || favItems.length === 0"
          ...
          <div v-else class="space-y-4">
            <div v-for="item in favItems" :key="item._id"
                 class="flex items-start gap-3 p-3 bg-gray-50 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200">
              <img v-if="item.productDetails && item.productDetails.image" 
                   :src="item.productDetails.image" 
                   :alt="item.productDetails.name"
                   class="imgshop">
              <div class="flex-grow">
                <h3 class="font-semibold text-sm mb-1">{{ item.productDetails ? item.productDetails.name : 'Unknown Product' }}</h3>
                <p class="text-sm mb-2">size: {{ item.size || 'N/A' }}</p>
                <div class="text-red-500 font-semibold text-sm mb-2">price: {{ item.productDetails ? item.productDetails.price : 'N/A' }}$</div>
        
                <div class="flex space-x-2">
                  <button class="btnmodal text-red-500 hover:bg-red-100" @click="removeFromFav(item._id)"
...
onMounted(async () => {
  try {
    ...
    error.value = 'Failed to load favorite items. Please try again later.';
  }
});
...
const removeFromFav = async (item) => {
  try {
    ...
    error.value = 'Failed to remove item from favorites. Please try again.';
  } finally {
    await modalStore.fetchFavItems();
  }
};
...
const favItems = computed(() => {
  try {
    return modalStore.favItems || [];
  } catch (err) {
    console.error('Error accessing favItems:', err);
    error.value = 'Error loading favorite items.';
    return [];
  }
});

const isFetchingFav = computed(() => {
  try {
    return modalStore.isFetchingFav;
  } catch (err) {
    console.error('Error accessing isFetchingFav:', err);
    return false;
  }
});

const isRemovingFromFav = computed(() => {
  try {
    return modalStore.isRemovingFromFav;
  } catch (err) {
    console.error('Error accessing isRemovingFromFav:', err);
    return false;
  }
});
</script>
